...
# Use gather() to take multiple columns and collape them into key-value pairs. Gather() moves wide data into long data. Will output a key column and a value column based on the genes.name column.
...
#First, filter() will select the rows from the data.table with the 6hpi timepoint. Then, datya will be arrang so that the gene with the highest expression will be on top, using arrange() on the 'value' variable.
...
#First, add two new columns with mutate(): one named time.value which is the time.point column but without the 'hpi'. Use str_replace to replace 'hpi' with nothing ''. Second one is named time.hpi, and concatenates 'hpi.' to the time.value column giving 'hpi.6.' 
...
#Then we want to create new columns of each of the timepoints. In time.point, there are 4 timepoints 6,12,18,24hpi but they are all in this single time.point column. We want each time point to be its own variable, and can use 'spread' to do this. 

#Note: the time.point and time.value columns must be removed before spreading in order to 'collapse' the values for each time point onto a single row. If not, each timepoint will have a value on its own row, so then can't subtract 24h-6h; values will be 'NA'.
...
# Calculate the summary statistics (mean and variance) using summarise(), after grouping with group_by(). 
...
# Now remove the genes that have a mean value of 0. These rows also have variance value of 0, and interfere with the final step. Is this necessary?

expr_data_sumd %>% filter(meanval > 0) %>% arrange(desc(meanval)) -> expr_data_final

#Now that we have the expression difference (expr.diff) and the vairance (expr.diff.var) we can use select(for columns) and filter(for rows) to identify the top 3 gene names with the highest expression difference (highest mean(expr.diff)) and lowest variance (lowest expr.diff.var). Not sure what to do with slice(), and why remove gene.name?

head(expr_data_final, n=3) %>% select(gene.name)
